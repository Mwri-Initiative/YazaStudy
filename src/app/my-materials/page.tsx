...
function MyMaterialsPageContent() {
  ...
  const filteredMaterials = materials.filter(material => {
    ...
    return ((material.title || '').toLowerCase().includes(q) || (material.description || '').toLowerCase().includes(q)) && (selectedSubject === 'all' || material.subject === selectedSubject)
  })

  const handleDownload = async (material: StudyMaterial) => {
    const url = material.downloadUrl
    if (!url) { alert('Download link coming soon!'); return }
    try {
  ...
  const handlePreview = async (material: StudyMaterial) => {
    ...
    if (!url) { alert('Material content not yet available.'); return }
    try {
      const storagePath = extractStoragePath(url)
      if (!storagePath) throw new Error('Could not parse storage path from URL')
      ...
      setViewerOpen(true)
    } catch (err) { console.error('Error securing PDF access:', err); alert('Failed to load secure material. Please try again or contact support.') }
  }
  ...
  return (
    <div className="min-h-screen relative py-8 px-4 sm:py-12 sm:px-6 lg:px-8">
      ...
      <div className="relative max-w-7xl mx-auto">
        <div className="mb-7 sm:mb-10"><div className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/10 px-3 py-1.5 text-[10px] font-extrabold uppercase tracking-[.16em] text-primary"><BookOpen className="h-3 w-3" /> Your library</div><h1 className="mt-4 text-3xl md:text-5xl font-black font-display text-text tracking-tight">My Study Materials</h1><p className="mt-3 text-sm sm:text-lg text-text-secondary max-w-2xl font-body">Your personal collection of premium MSCE resources. Ready whenever you are.</p></div>
...
export default function MyMaterialsPage() {
  return <Suspense fallback={<div className="min-h-screen px-4 py-12 sm:px-6"><div className="mx-auto max-w-7xl"><div className="h-10 w-56 animate-pulse rounded-xl bg-white/5" /><div className="mt-8 h-24 animate-pulse rounded-3xl bg-white/5" /><div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><div className="h-72 animate-pulse rounded-3xl bg-white/5" /><div className="h-72 animate-pulse rounded-3xl bg-white/5" /><div className="h-72 animate-pulse rounded-3xl bg-white/5" /></div></div></div>}><MyMaterialsPageContent /></Suspense>
}
